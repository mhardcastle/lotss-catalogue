...
#
# this version for DR2 with experimental multiprocessing for speed
...
def match_click_ellipse(ra,dec,cat,click,nsource):
...
def match_opt(ra,dec,click,cat):
...
print('Reading optical catalogue')
...
rast='RA'
decst='DEC'
print('Adding sizes to optical table')
...
def process_source(subj):
    ...
    if num_assoc>0:
        ...
        for index,news in assocs_s.iterrows():
            classesa,lista,assocpos=match_click_ellipse(ra,dec,subcat,news,source_name)
    ...
    if num_ids>0:
        ...
        for index,news in ids_s.iterrows():
            ...
            # Optical match
            poss,ora,odec,user=match_opt(ra,dec,news,osubcat)
...
p=Pool(96)
#for _ in tqdm(p.imap(process_source,subs_unique),total=len(subs_unique)):
#    pass
for _ in tqdm(map(process_source,subs_unique),total=len(subs_unique)):
    pass   
