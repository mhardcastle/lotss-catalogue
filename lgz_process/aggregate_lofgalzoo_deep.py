...
def match_opt(click,cat):
    ...
    pdec=np.nan
    for s in cat:
        dist=np.sqrt((np.abs(rx-s[rast])*np.cos(dec*np.pi/180.0))**2.0+(np.abs(dy-s[decst])**2.0))
        if dist<thres:
            poss=str(s[idname])
            pra=s[rast]
            pdec=s[decst]
...
# We assume this is running in one of the lgz deep fields directories
...
for subj in subs_unique:
    ...
    if num_ids>0:
        ...
        osubcat=opt[(np.abs(opt['ALPHA_J2000']-ra)*np.cos(dec*np.pi/180.0)<rsize) & (np.abs(opt['DELTA_J2000']-dec)<dsize)]
