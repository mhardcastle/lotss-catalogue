...
# experimental multiprocessing in main loop
...
def match_click_ellipse(ra,dec,ascale,pscale,cat,click,nsource):
        ...
        for s in cat:
            if s['Source_Name'] == nsource:
                ...
                if(pointInEllipse(xcheck,realy,x,pixim-y,2.0*pscale*s['Maj'],2.0*pscale*s['Min'],s['PA']+90.0) is True):
                    matched_orig=True
        if not matched_orig:
            if debug: print('Warning: click not matched to any ellipse',x,y,rx,dy)
    return classesa,lista,assocpos

def match_opt(ra,dec,ascale,click,cat):
...
def tally_votes_weight(matches,weights,tweight):

     # For each unique ID match, sum the number of weighed votes and
     # compare to the total weights. Weights are provided in a list
    ...
    for m in uniquematches:
        numioccur=matches.count(m)
        wsum=np.sum([weights[i] for i,n in enumerate(matches) if n==m])
...
def process_source(subj):

    # We amend local copies of all the dictionaries and lists updated
    # by the original single-threaded code and then return a tuple of
    # them that can be merged with the global version.

    # These are:
    #     dicts: coordsdict, fluxdict, sizedict, mindict, padict, seendict, probdict, ocoordsdict, badclickdict 
    #     lists: classa, alist, subjs_noassoc, ilist, multlist, subjs_mult, subjs_noids
    ...
    if len(rowt)>0:
        ...
        print('Warning: source',subj,'does not exist in components table')
        coordsdict.update({source_name:(0,0)})
        fluxdict.update({source_name:0})
        sizedict.update({source_name:0})
        mindict.update({source_name:0})
        padict.update({source_name:0})
    ...
    if num_probs>0:
        ...
        otherfrac=float(ocount)/float(numseen)
        missfrac=float(mcount)/float(numseen)

        probdict.update({source_name:(artfrac,blendfrac,zoomfrac,otherfrac,missfrac)})    
    ...
    if num_assoc>0:
        ...
        for index,news in assocs_s.iterrows():
            classesa,lista,assocpos=match_click_ellipse(ra,dec,ascale,pscale,subcat,news,source_name)
    ...
    if num_ids>0:
        ...
        for index,news in ids_s.iterrows():
            ...
            # Optical match
            poss,ora,odec,user=match_opt(ra,dec,ascale,news,osubcat)
            w=weight(user)
        ...
        goods=tally_votes_weight(imatches,weights,tweight)
        numigood=len(goods)
...
# No IDs
    ...
    return coordsdict, fluxdict, sizedict, mindict, padict, seendict, probdict, ocoordsdict, assocdict, badclickdict, classa, alist, subjs_noassoc, ilist, multlist, subjs_mult, subjs_noids # local versions of dictionaries
        
def make_set(classed):
    newset=[]
...
def collate_votes(subj):
    ...
    if nelem>23:
        print(subj,'has too many unique elements, cannot count votes')
        p_toobig.append(subj)
    else:
    ...
    return p_toobig,p_setscore,left
...
with open('/beegfs/lofar/mjh/rgz/consensus_frac.json') as f:
    frac=json.load(f)
...
sourcecat='source_lr.fits' # has the a, b names
...
new_subs_unique=list(subs_unique)
nct=0

p=Pool(ncpu)
for result in tqdm(p.imap(process_source,subs_unique),total=len(subs_unique)):
    p_coordsdict, p_fluxdict, p_sizedict, p_mindict, p_padict, p_seendict, p_probdict, p_ocoordsdict, p_assocdict, p_badclickdict, p_classa, p_alist, p_subjs_noassoc, p_ilist, p_multlist, p_subjs_mult, p_subjs_noids = result
    coordsdict.update(p_coordsdict)
    fluxdict.update(p_fluxdict)
    sizedict.update(p_sizedict)
    mindict.update(p_mindict)
    padict.update(p_padict)
    seendict.update(p_seendict)
    probdict.update(p_probdict)
    ocoordsdict.update(p_ocoordsdict)
    assocdict.update(p_assocdict)
    badclickdict.update(p_badclickdict)
    classa+=p_classa
    alist+=p_alist
    subjs_noassoc+=p_subjs_noassoc
    ilist+=p_ilist
    multlist+=p_multlist
    subjs_mult+=p_subjs_mult
    subjs_noids+=p_subjs_noids
...
p=Pool(ncpu)
listofsets=[]
for result in tqdm(p.imap(make_set,classinassocs),total=len(classinassocs)):
    listofsets.append(result)
...
for assocsrc in tqdm(alist):
    if assocsrc['Assoc_Name'] in subs_unique:
...
p=Pool(ncpu)
for result in tqdm(p.imap(collate_votes,new_subs_unique),total=len(new_subs_unique)):
    p_toobig,p_setscore,left=result
    toobig+=p_toobig
    setscore+=p_setscore
    for item in left:
        allgoodsets.add(frozenset(item))

p.close()
p.join()
p.terminate()
        
# The set "allgoodsets" should now contain the largest set with a score>2.0/3.0 containing each target source, where such a set exists. 
...
for finalset in tqdm(allgoodsets):
    ...
    if setsubjs>0:
        ...
        if debug: print('badclicklist is',badclicklist)
        ...
        ncomps=len(finalset)
        finalcat.append({'Source_Name':setname,'RA':ramean,'Dec':decmean,'OptID_Name':optid, 'optRA':optra, 'optDec':optdec,'Flux':nflux,'Size':estsize,'Assoc':ncomps, 'Assoc_Qual':newscore[0][1],'ID_Qual':qual,'Compoverlap':oflag,'Art_prob':apb,'Blend_prob':bpb,'Zoom_prob':zpb,'Other_prob':hpb,'Imagemissing_prob':imb,'Badclick':badclick})
...
print('Exclude members of sets from single sources')
...
for subj in tqdm(new_subs_unique):
    ...
    if subj in toobig:
        if debug: print('Forcing zoom to 1.0 for',subj)
        zpb=1.0
    ...
    if subj in overlap:
        oflag=1
    finalcat.append({'Source_Name':subj,'RA':nra,'Dec':ndec,'OptID_Name':optid, 'optRA':optra, 'optDec':optdec,'Flux':nflux,'Size':nmajsiz,'Assoc':0, 'Assoc_Qual':np.nan,'ID_Qual':qual,'Compoverlap':oflag,'Art_prob':apb,'Blend_prob':bpb,'Zoom_prob':zpb,'Other_prob':hpb,'Imagemissing_prob':imb,'Badclick':badclickdict[subj]})
...
write_fits_out(['Source_Name','RA','Dec','OptID_Name','optRA','optDec','Flux','Size','Assoc','Assoc_Qual','ID_Qual','Compoverlap','Art_prob','Blend_prob','Zoom_prob','Other_prob','Imagemissing_prob','Badclick'],finalcat,"weighted-LGZ-cat.fits")
write_fits_out(['Comp_Name','Assoc','Source_Name','Comp_RA','Comp_DEC','Comp_Flux','Comp_Maj','Comp_Min','Comp_PA'],compcat,'weighted-LGZ-comps.fits')

print('Multlist contains',len(multlist),'sources')
if len(multlist)>0:
    #'Source_Name':subj,'Opt_id':nam,'Numclass':numseen,'Quality':tqual
    write_fits_out(['Source_Name','Opt_id','Numclass','Quality'],multlist,'weighted-LGZ-multiple.fits')
