...
dir='/data/lofar/mjh/hetdex_v4/toclassify'
sample='large_faint_toclassify'
options=('Send to LGZ', 'Accept ML match', 'No good match', 'Too zoomed in','Artefact')
...
while i<len(t) and i<end:
    ...
    instructions=[str(name)+' (%i of %i) (%s)' % (i,len(t),desc),'']
    instructions+=['(%i) %s' % (j+1,s) for j,s in enumerate(options)]
    instructions+=['','LEFT: back one','RIGHT: forward one', 'Q: quit','',"Your choice:"]
    attrs=[curses.color_pair(1)]+[None,]*(len(instructions)-1)
    if np.isnan(r['LR_ra']):
        attrs[3]=curses.A_REVERSE
    for j in range(len(instructions)):
        l=instructions[j]
        a=attrs[j]
        if a is not None:
            stdscr.addstr(6+j,2,l,a)
        else:
            stdscr.addstr(6+j,2,l)
    stdscr.refresh()

    valid=False
    quit=False
    while not(valid):
        key = stdscr.getch(6+j,2+len(instructions[-1])+1)
