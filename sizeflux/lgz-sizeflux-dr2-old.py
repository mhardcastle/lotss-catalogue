...
def FloodMaskBoth(source_name, cinc, cexc, hdu,rmsthres, p):
    ...
    for i,src in enumerate(cinc):
        ...
        ellipses.append((i,(sizey,sizex),xp,yp,majpix,minpix,sang+90.0))
    ...
    masks=p.map(amewrapper,ellipses)
    nmask=np.sum(masks,axis=0)
    #nmask=m.narray
    ...
    for i,row in enumerate(cexc):
        ...
        ellipses.append((xp,yp,majpix,minpix,comp_pa+90.0))
    ...
    if len(ellipses)>0:
        ...
        masks=p.map(em,ellipses)
        print('Summing')
        exclude_mask=np.sum(masks,axis=0)
        print('Done')
    else:
        exclude_mask=np.zeros((sizey,sizex),dtype=np.short)
...
def length3(a):
    ...
    xv=xv[~np.isnan(a)]
    yv=yv[~np.isnan(a)]
    return np.max(pdist(np.vstack([xv,yv])))
...
if __name__=='__main__':
    ...
    for src in incat:
        ...
        if ff==0:
            ...
            print("Lengths of included and excluded comps before filter are:",len(catinc),len(dcatexc))
            assert(len(catinc))
            catexc=dfilt(dcatexc,ra,dec,1.5*size)
            print("Lengths of included and excluded comps are:",len(catinc),len(catexc))

            floodmask,floodim=FloodMaskBoth(name,catinc,catexc,cutout,rthres,pool)

            # Calculate flux and size
            print('Get flux and size')
            fl,sz=getfluxsize(floodim)
            print('Size is',sz,'pixels')
            print('Done!')

            totflux=fl/beamarea
            totsize=(sz*pdel)*3600.0

            outcat.append({'Source_Name':name,'RA':ra,'DEC':dec,'Total_flux_LoTSS':influx,'New_flux':totflux,'Maj_LoTSS':maj*3600.0,'LGZ_Size_LoTSS':lgz,'New_size':totsize})

    write_fits_out(['Source_Name','RA','DEC','Total_flux_LoTSS','New_flux','Maj_LoTSS','LGZ_Size_in','New_size'],outcat,'LM-size-flux.fits')
